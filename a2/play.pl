...
mmeval(1,St,Val,BestMv,D,SeF) :- 
 ...
 write('Evaluating '), write(L), write(' moves at Plyr 1 depth '), writeln(D), 
  ...
  evalMoves(1,St,MvList,B,null,Val,BestMv,D,0,SeI), % Best so far set to lowerbnd  %Null = null
...
mmeval(2,St,Val,BestMv,D,SeF) :- 
  moves(2,St,MvList), write(MvList), nl,!,
 length(MvList,L), 
 write('Evaluating '), write(L), write(' moves for Plyr 2 at depth '), writeln(D), 
  ...
  evalMoves(2,St,MvList,B,null,Val,BestMv,D,0,SeI), % Best so far set to upperbnd  %Null = null
  SeF is SeI + 1. 
...
%  write('evalMoves 1: '), write(Mv), write(' D='), write(D), write(' S='), write(Se), nl, showState(NewSt),
  Dnew is D - 1, 
...
evalMoves(2,St,[Mv|Rest],ValSoFar,MvSoFar,Val,BestMv,D,Se,SeF) :- 
  ...
  write('evalMoves 2: '), write(Mv), write(' D='), write(D), write(' S='), write(Se), nl, showState(NewSt),
  Dnew is D - 1, 
  mmeval(NextPlyr,NewSt,MvVal,_,Dnew,SeI),  !,
