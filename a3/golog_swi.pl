...
% an iterative deepening planning procedure
%
% To use it you must define the goal fluent goal(s) and
% the forward filtering fluent acceptable(a,s)

proc(idPlan(N), idPlan(0,N)).

proc(idPlan(M,N), dfPlan(M) #
       ?(M < N) : pi(m1, ?(m1 is M + 1) : idPlan(m1,N))).

proc(dfPlan(N),
      ?(goal) #
      ?(N > 0) : pi(a, ?(acceptable(a)) : a) : 
             pi(n1, ?(n1 is N - 1) : dfPlan(n1))).
